import { routineItems, routineIcon, routineStatus, selectedRoutine } from "./checklist.js";
...
function drawClock() {
  ...
  face.innerHTML = `<circle class="clock-ring" cx="200" cy="200" r="162"/><path class="clock-arc" d="M${start.x} ${start.y} A162 162 0 0 1 ${end.x} ${end.y}"/>${ticks}${numbers}`;
  document.getElementById("clock-events").innerHTML = items.map((item) => {
    const position = point(item.minutes / 2, 162);
    return `<button type="button" class="clock-event" data-select-routine="${item.id}" style="left:${position.x / 4}%;top:${position.y / 4}%">${routineIcon(item)}</button>`;
  }).join("");
}
...
function renderClock(ts, replay = false) {
  ...
  document.getElementById("clock-selection").textContent = selected.label + " · " + selected.time;
  for (const item of routineItems()) {
    const button = document.querySelector('.clock-event[data-select-routine="' + item.id + '"]');
    button.className = "clock-event " + item.status;
    button.setAttribute("aria-pressed", String(selectedRoutine().id === item.id));
    button.setAttribute("aria-label", item.label + ", " + item.time + ". " + routineStatus(item) + ". Show reminder.");
    button.title = item.label + " · " + item.time;
  }
