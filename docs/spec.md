# Brain Buddy — Build specification

See the [website guide](app.md) for the current UI and local server setup, and [deployment guide](deployment.md) for publishing. Source paths in this document are relative to the repository root.
...
- **Live mode** — same files served locally (`python3 tools/message_server.py` from the repository root) during the demo, polling the ESP32 over the LAN via HTTP. This is what judges see live.
- **Replay mode** — the GitHub Pages deployment, which loads `data/demo.json` (a captured event log) and animates it. This is what judges see when they open the link later. Auto-fallback: if the ESP32 fetch fails, switch to replay; the clock and date indicate recorded time.
...
├── app/                       # website and GitHub Pages artifact root
│   ├── index.html
│   ├── css/style.css
│   ├── js/                    # UI, messaging, routine, map, and API modules
│   ├── data/demo.json         # recorded bracelet events
│   ├── icons/
│   ├── vendor/leaflet/        # local map library and license
│   ├── manifest.json
│   └── sw.js
├── docs/
│   ├── README.md              # documentation index
│   ├── spec.md                # build specification
│   ├── app.md                 # website guide
│   ├── deployment.md          # local serving and GitHub Pages
│   └── img/wiring.svg         # documentation illustration
├── firmware/                  # PlatformIO firmware, modules, and tests
├── hardware/                  # wiring guide and bill of materials
├── tools/                     # message server, contract checks, capture tools
...
`app/data/demo.json` is exactly an `/events` array plus an initial state — so `tools/capture_demo.py` can record a real session and replay mode needs zero special-casing.
...
5. **Repo ↔ Pages:** `.github/workflows/pages.yml` publishes only `app/` when changes reach `main`, with GitHub Pages configured to use GitHub Actions. `capture_demo.py` → commit `demo.json` → the public page shows your actual dry-run data.
...
| Dashboard | 5 | app/ everything, hand-typed demo.json first |
