#!/usr/bin/env python3
"""Record a real session off the wristband into docs/data/demo.json. Stdlib only.
...
demo.json is what the dashboard falls back to when there is no band on the
network - the venue WiFi, the battery, the five minutes before the judges
arrive. Capturing it from real hardware rather than writing it by hand is the
difference between a demo that shows the product and one that shows a mock-up.
...
def schedule_ids(base):
    """Ids the band is actually running, which may be a routine the dashboard pushed."""
    try:
...
def follow(base, minutes, since):
    """Poll like the dashboard does, collecting events as they happen."""
...
def main():
    ...
    parser.add_argument("--out", default=str(ROOT / "docs/data/demo.json"))
    ...
    if problems:
        # A snapshot that breaks the contract would make the offline dashboard
    ...
    if state["ts"] < 1700000000:
        print("FAIL the device clock is unset, so every timestamp would be wrong.\n"
              "     Open the dashboard against the band, or run "
    ...
    print("     The dashboard replays this whenever no band answers.")
    return 0
